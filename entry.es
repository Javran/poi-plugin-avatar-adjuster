...
window.avatarAdjusterWindow = remote.getCurrentWindow()

const rememberSize = _.debounce(() => {
  const b = window.avatarAdjusterWindow.getBounds()
  config.set('plugin.avatarAdjusterWindow.bounds', b)
}, 5000)

window.avatarAdjusterWindow.on('move', rememberSize)
window.avatarAdjusterWindow.on('resize', rememberSize)
